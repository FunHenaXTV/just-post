...
class GetPostInfo final : public userver::server::handlers::HttpHandlerBase {
 public:
  ...
  std::string HandleRequestThrow(
    ...
    if (post_id.size()) {
      ...
      auto result = pg_cluster_->Execute(
          ...
          "SELECT just_post_schema.users.email, post_body, "
          "to_char(date_of_post, 'YYYY-MM-DD HH24:MI:SS+03:00')"
      ...
      json_post_info += "\"" + row.date_of_post.substr(0, 10) + "T" +
                        row.date_of_post.substr(11, row.date_of_post.size()) +
                        "\"";
      json_post_info += "}";
...
void AppendGetPostInfo(userver::components::ComponentList& component_list) {
  component_list.Append<GetPostInfo>();
}
